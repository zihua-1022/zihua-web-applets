...
function reviewImages(num){
    ...
    $.ajax({
      type: "GET",
      url: "/image/reviewVideo/id=" + ID,
      dataType: "json",
      success: function (res) {
        xadmin.open(''+ res[0].image_title +'','../'+ res[0].data_path +'','','',true)
      },
      error: function () {
        layer.alert('学习视频请求失败', {
...
{
  let ID=num;
  layer.tips('学习资源正在下载中...', '#export'+ ID +''); 
//   layer.tips('学习资源正在下载中...', '#export'+ ID +'', {
//     tips: [1, '#3595CC'],
//     time: 4000
//   });
        ...
        for (let i = 0; i < 4; i++) 
        {
            if ((i + j * 4) < imageData.length)   //onchange=chkChange('+i+')name="jcheck'+i+'"  bu id="see'+i+'" onclick="reviewImages('+i+')"
            {
              ...
              content ='<tr id="PageNavId' + j + '">\
                  ...
                  <a style="background-color: #009688;" type="button" id="export'+imageData[i + j * 4][1]+'" onclick="exportTxt('+imageData[i + j * 4][1]+')"\
                  href="/export_data/content=实验数据&txtid=/'+imageData[i + j * 4][1]+'" class="layui-btn layui-btn-normal">下载</a>\
                  </td>\
                </tr>';
