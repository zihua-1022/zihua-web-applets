...
# 实例化一个蓝图
fileoperaapp=Blueprint('fileopera',__name__,template_folder="../templates",static_folder="../static")


@fileoperaapp.route('/out_info',strict_slashes=True)
def out_info():
    return render_template('out_info.html')


## 信息导出按时间查找数据(修改过)
@fileoperaapp.route('/searchtime',strict_slashes=True)  # 查询数据使用GET请求
def searchtime():
