...
function getData(typee) {
    if (typee == '数据管理') {
        $.getJSON("/image/get_imagedata", function (imageData) {
            $('#page').empty();
            inforData = imageData;
            showData(typee);//展示数据
        });
        ...
        $.getJSON("/user/get_userdata", function (userData) {
            $('#page').empty();
            inforData = userData;
            showData(typee);//展示数据 
        });
        ...
        $.getJSON("/logs/get_logsdata", function (logsData) {
            $('#page').empty();
            inforData = logsData;
            showData(typee);//展示数据
        });
...
$(document).ready(function () {
    layui.use('laydate', function searchTime(){
        ...
        elem: '#time_collection' ,//指定元素
            ...
            if (dropdownVal == '数据管理') {
                types = 'upload_img';
                $.getJSON("/fileopera/searchtime", { time: time, types: types }, function (data) {
                    $('.page').empty();
                    inforData = data;
